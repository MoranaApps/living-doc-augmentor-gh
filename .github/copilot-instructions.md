...
- Must lint with Pylint and Ruff (ignore tests/).
- Must type-check with mypy (strict mode).
- Must keep coverage ≥ 95% when running pytest-cov.
- Must keep cyclomatic complexity ≤ B (radon).
- Must pass pip-audit with zero known vulnerabilities.
...
Repo additions
- Project name: living-doc-augmentor-gh
- Entry points: action.yml, main.py
- Core package: living_doc_augmentor/
- Input layer: CLI args + INPUT_* env vars
- Logging wiring: structured log format [LEVEL] [TIMESTAMP] [COMPONENT] MESSAGE
- Contract-sensitive outputs: code_augmentations.json; annotation violation strings; exit codes (0/1/2)
- Commands:
  - python3 -m venv .venv

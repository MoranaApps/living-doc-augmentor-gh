...
- Domain-specific high-risk areas
  - Annotation detection: regex patterns, comment parsing, placement rules.
  - Configuration loading: Pydantic validation, schema versioning, backward compatibility.
  - Exit codes: do not change existing failure exit codes (0/1/2) unintentionally.
  - Contract-sensitive outputs: code_augmentations.json schema; violation strings; `$GITHUB_STEP_SUMMARY` format.
